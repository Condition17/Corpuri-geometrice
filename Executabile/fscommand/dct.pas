    ...
    for i := 1 to nr do
      ...
      begin
        ...
        if img[i] <> '' then
         ...
         gigi.Visible:=false; Im.Visible:=false;
      end;
    if not gasit then
    begin
      gigi.Visible := true;
      Im.Visible := true;
      Memo1.Text :=
        'Termenul nu exista!Daca vrei sa-l adaugi in dictionar, scrie in acesta fereastra explicatia, apoi da click pe butonul "Adaugã".'
    end;
  end
  else
    Memo1.Text :=
...
begin
  ...
  if (cauta.Text <> '') then
    if (Memo1.Text <> '') and (Memo1.Text <>
        'Termenul nu exista!Daca vrei sa-l adaugi in dictionar, scrie in acesta fereastra explicatia, apoi da click pe butonul "Adaugã".') then
      valid := true
    else
      valid := false;
  if valid then
